...
See [docs/lights-optimizer.md](docs/lights-optimizer.md) for the category table, provenance/manual-lock model, presence contract, special-case blocks, reason vocabulary, and tunable env vars. Per-room policy spec: [docs/lighting-policy-by-room.md](docs/lighting-policy-by-room.md).

## Presence Platform

Local Zigbee room-occupancy that drives the optimizer (no Home Assistant). Zigbee sensors (Aqara FP300 mmWave in the living room; SONOFF SNZB-03PR2 PIR in bedrooms/halls/bathrooms/WC) → SLZB-06U coordinator (network, TCP `:6638`) → **Zigbee2MQTT** (`zigbee2mqtt` service) → **Presence Engine** (`scripts/presence_engine.py`, `presence` service) which normalizes every vendor payload into one per-room model — `presence/<room>` (retained MQTT) + the `presence` InfluxDB measurement — that `lights_optimizer` consumes via `presence_for_room()`. Room mapping: `config/presence_rooms.json` (device friendly_name → room, hot-reloaded) ↔ `LIGHT_ROOM` in the optimizer. Both Zigbee containers (`zigbee2mqtt` + `presence`) now start by default with `docker compose up -d` — the coordinator IP is baked into the compose config (`SLZB_HOST` default `192.168.1.218`); override `SLZB_HOST` if it moves. The optimizer already consumes presence: per-room auto-on (motion+dark) and vacancy-off activate as each room's sensor is added, with no code change.

See [docs/presence-setup.md](docs/presence-setup.md) for hardware, coordinator/pairing steps (FP300 must be in Zigbee mode), room mapping, the normalized event model, tuning, and troubleshooting.
...
A fourth action, `light_request` (light_id + duration_s + only_if_dark), is **signal-only**: it writes a `light_override` hold but does NOT publish to `/set` — the `lights-optimizer` is the sole controller of that light and actuates it (using command provenance so it never overrides a manual on). The default ruleset pairs front-door person detection with an announcement (5-min image overlay) AND a `light_request` for the front porch (idx 47, 5 min): the porch no longer auto-ons at dusk (removed by request) — it's lit only by detection, via the optimizer, and turned off when the detection window ends.
